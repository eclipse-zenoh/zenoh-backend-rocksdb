pipeline {
  ...
  environment {
      LABEL = get_label()
      DOWNLOAD_DIR="/home/data/httpd/download.eclipse.org/zenoh/zenoh-backend-rocksdb/${LABEL}"
  ...
  stages {
    stage('Checkout Git TAG') {
      steps {
        ...
        checkout([$class: 'GitSCM',
                  ...
                  gitTool: 'Default',
                  submoduleCfg: [],
                  userRemoteConfigs: [[url: 'https://github.com/eclipse-zenoh/zenoh-backend-rocksdb.git']]
                ])
      }
    }
    stage('Update Rust env') {
      steps {
        sh '''
        env
        echo "Building zenoh-backend-rocksdb-${LABEL}"
        rustup update
        '''
      }
    }
    ...
    stage('MacOS build') {
      ...
      steps {
        ...
        cargo build --release
        cargo test --release
        tar -czvf zenoh-backend-rocksdb-${LABEL}-macosx${MACOSX_DEPLOYMENT_TARGET}-x86-64.tgz --strip-components 2 target/release/*.dylib
        '''
      }
    }
    ...
    stage('x86_64-unknown-linux-musl build') {
      ...
      steps {
        ...
        tar -czvf zenoh-backend-rocksdb-${LABEL}-x86_64-unknown-linux-musl.tgz --strip-components 3 target/x86_64-unknown-linux-musl/release/*.so
        '''
      }
    }
    ...
    stage('x86_64-unknown-linux-gnu build') {
      ...
      steps {
        ...
        tar -czvf zenoh-backend-rocksdb-${LABEL}-x86_64-unknown-linux-gnu.tgz --strip-components 3 target/x86_64-unknown-linux-gnu/release/*.so
        '''
      }
    }
    ...
    stage('i686-unknown-linux-gnu build') {
      ...
      steps {
        ...
        tar -czvf zenoh-backend-rocksdb-${LABEL}-i686-unknown-linux-gnu.tgz --strip-components 3 target/i686-unknown-linux-gnu/release/*.so
        '''
      }
    }
    ...
    stage('aarch64-unknown-linux-gnu build') {
      ...
      steps {
        ...
        tar -czvf zenoh-backend-rocksdb-${LABEL}-aarch64-unknown-linux-gnu.tgz --strip-components 3 target/aarch64-unknown-linux-gnu/release/*.so
        '''
      }
    }
    ...
    stage('x86_64-pc-windows-gnu build') {
      ...
      steps {
        ...
        zip zenoh-backend-rocksdb-${LABEL}-x86_64-pc-windows-gnu.zip --junk-paths target/x86_64-pc-windows-gnu/release/*.dll
        '''
      }
    }
    ...
    stage('i686-pc-windows-gnu build') {
      ...
      steps {
        ...
        zip zenoh-backend-rocksdb-${LABEL}-i686-pc-windows-gnu.zip --junk-paths target/i686-pc-windows-gnu/release/*.dll
        '''
      }
    }
        ...
        sshagent ( ['projects-storage.eclipse.org-bot-ssh']) {
          sh '''
            if [[ ${GIT_TAG} == origin/* ]]; then
              ...
              echo "https://github.com/eclipse-zenoh/zenoh-backend-rocksdb/tree/${COMMIT_ID}" > _git_commit_${COMMIT_ID}.txt
    ...
    stage('Publish zenoh-macosx to download.eclipse.org') {
      ...
      steps {
        sshagent ( ['projects-storage.eclipse.org-bot-ssh']) {
          sh '''
            ssh genie.zenoh@projects-storage.eclipse.org mkdir -p ${DOWNLOAD_DIR}
            scp zenoh-backend-rocksdb-${LABEL}-*macosx*.tgz genie.zenoh@projects-storage.eclipse.org:${DOWNLOAD_DIR}/
          '''
        }
      }
    }
    ...
    stage('Publish zenoh-x86_64-unknown-linux-musl to download.eclipse.org') {
      ...
      steps {
        sshagent ( ['projects-storage.eclipse.org-bot-ssh']) {
          sh '''
            ssh genie.zenoh@projects-storage.eclipse.org mkdir -p ${DOWNLOAD_DIR}
            scp zenoh-backend-rocksdb-${LABEL}-x86_64-unknown-linux-musl.tgz genie.zenoh@projects-storage.eclipse.org:${DOWNLOAD_DIR}/
          '''
        }
      }
    }
    ...
    stage('Publish zenoh-x86_64-unknown-linux-gnu to download.eclipse.org') {
      ...
      steps {
        sshagent ( ['projects-storage.eclipse.org-bot-ssh']) {
          sh '''
            ssh genie.zenoh@projects-storage.eclipse.org mkdir -p ${DOWNLOAD_DIR}
            scp zenoh-backend-rocksdb-${LABEL}-x86_64-unknown-linux-gnu.tgz genie.zenoh@projects-storage.eclipse.org:${DOWNLOAD_DIR}/
            if [[ ${GIT_TAG} != origin/* ]]; then
    ...
    stage('Publish zenoh-i686-unknown-linux-gnu to download.eclipse.org') {
      ...
      steps {
        sshagent ( ['projects-storage.eclipse.org-bot-ssh']) {
          sh '''
            ssh genie.zenoh@projects-storage.eclipse.org mkdir -p ${DOWNLOAD_DIR}
            scp zenoh-backend-rocksdb-${LABEL}-i686-unknown-linux-gnu.tgz genie.zenoh@projects-storage.eclipse.org:${DOWNLOAD_DIR}/
            if [[ ${GIT_TAG} != origin/* ]]; then
    ...
    stage('Publish zenoh-x86_64-pc-windows-gnu to download.eclipse.org') {
      ...
      steps {
        sshagent ( ['projects-storage.eclipse.org-bot-ssh']) {
          sh '''
            ssh genie.zenoh@projects-storage.eclipse.org mkdir -p ${DOWNLOAD_DIR}
            scp zenoh-backend-rocksdb-${LABEL}-x86_64-pc-windows-gnu.zip genie.zenoh@projects-storage.eclipse.org:${DOWNLOAD_DIR}/
          '''
        }
      }
    }
    ...
    stage('Publish zenoh-i686-pc-windows-gnu to download.eclipse.org') {
      ...
      steps {
        sshagent ( ['projects-storage.eclipse.org-bot-ssh']) {
          sh '''
            ssh genie.zenoh@projects-storage.eclipse.org mkdir -p ${DOWNLOAD_DIR}
            scp zenoh-backend-rocksdb-${LABEL}-i686-pc-windows-gnu.zip genie.zenoh@projects-storage.eclipse.org:${DOWNLOAD_DIR}/
          '''
        }
      }
    }
